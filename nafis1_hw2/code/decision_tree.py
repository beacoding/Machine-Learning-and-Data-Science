import numpy as np
import decision_stump as stump
...
def fit(X, y, max_depth):
    ...
    splitModel = stump.fit(X, y)

    if max_depth <= 1 or splitModel["splitVariable"] is None:
        ...
        model["splitModel"] = splitModel
        model["subModel1"] = fit(X[splitIndex1], y[splitIndex1], max_depth-1)
        model["subModel0"] = fit(X[splitIndex0], y[splitIndex0], max_depth-1)
